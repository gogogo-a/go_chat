...
func (m *messageService) UploadAvatar(c *gin.Context) (string, int) {
	...
	for key, _ := range mForm.File {
		...
		zlog.Info(ext)
		// 确保目录存在
		avatarPath := config.GetConfig().StaticAvatarPath
		if err := os.MkdirAll(avatarPath, 0755); err != nil {
			zlog.Error(err.Error())
			return constants.SYSTEM_ERROR, -1
		}
		localFileName := avatarPath + "/" + fileHeader.Filename
...
func (m *messageService) UploadFile(c *gin.Context) (string, int) {
	...
	for key, _ := range mForm.File {
		...
		zlog.Info(ext)
		// 确保目录存在
		filePath := config.GetConfig().StaticFilePath
		if err := os.MkdirAll(filePath, 0755); err != nil {
			zlog.Error(err.Error())
			return constants.SYSTEM_ERROR, -1
		}
		localFileName := filePath + "/" + fileHeader.Filename
		out, err := os.Create(localFileName)
		if err != nil {
			zlog.Error(err.Error())
			return constants.SYSTEM_ERROR, -1
